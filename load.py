...
def create_model(num_classes, dropout_rate):
    model = tf.keras.applications.InceptionV3(include_top=True, weights=None, input_shape=(256, 256, 3), classes=num_classes)
    model.compile(optimizer="adam", loss="categorical_crossentropy", metrics=["accuracy"])

    return model


# Class labels(always in alphanumeric order)
classes = [
...
with open("predictions.csv", "w") as file:
    ...
    for filename in os.listdir("data/testing_data"):
        ...
        input_arr = tf.keras.preprocessing.image.img_to_array(image) / 255  # scaling
        input_arr = np.array([input_arr])  # convert single image to a batch

        prediction = model.__call__(input_arr).numpy()[0]  # Use __call__ on single element to save time
